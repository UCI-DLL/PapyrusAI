...
useEffect(() => {
  ...
  // Make sure it exists, it's from the AI, and it has finished streaming
  if (lastMessage && lastMessage.role === 'assistant' && lastMessage.finished) {
    
    // Create the native browser speech request
    const utterance = new SpeechSynthesisUtterance(lastMessage.content);
    
    // Optional: change the voice, pitch, or speed here
    // utterance.rate = 1.0; 
    
    // Speak the message
    window.speechSynthesis.speak(utterance);
    
    // 5. Reset UI state back to 'IDLE' so the user knows they can speak again
    setTurnState('IDLE'); 
  }
