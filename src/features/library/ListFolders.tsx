    ...
    function handleFilter(e: React.FormEvent) {
        ...
        if ((filters.starred as string) === StarredOptions.All) {
            ...
            orgFilteredFolderList = orgFilteredFolderList.filter(
                (x: FolderType) =>
                    starred.folders &&
                    starred.folders.some((y) => y.folderId === x.id)
            );
        ...
        if (filters.startDate) {
            orgFilteredFolderList = orgFilteredFolderList.filter(
                (folder: FolderType) => {
                    var date = new Date(
                        parseInt(folder.id.substring(0, 13), 10)
                    );
        ...
        <div className="space-y-6">
            {props.noShowMenu && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
                    To add an asset (including prompts and documents) to your
                    module, navigate to the folder in which the asset is hosted.
                    <span className="italic">
                        {" "}
                        Note: Assets must be created or uploaded in the Library
                        before they can be added to a module.
                    </span>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {orgFilteredFolderList.map((folder: FolderType, i) => {
                    if (
                        props.disableFolderId &&
                        props.disableFolderId === folder.id
                    ) {
                        return null;
                    }
                    return (
